...
1. 在 GitHub Releases 下载 `Nanpad-1.0.1-setup.exe`，适用于 Windows x64。
...
| 浏览器插件 | Chrome / Edge 登录自动采集（默认开启）、手动采集、配对本机、核对后加密保存；应用内一键下载插件            |
| 邮箱       | 文件夹收纳、IMAP 阅读与搜索、安全 HTML / Markdown 排版、头像、附件、SMTP 写信及加密草稿                 |
| 凭据库     | scrypt 派生密钥、AES-256-GCM 加密，支持分组收纳、锁定和修改主密码                                        |
...
| 性能与存储 | 检索索引签名复用（重复提问提速约 36 倍）、邮件会话缓存、图片授权延续、「设置 → 存储」缓存管理        |
...
可以直接问「我有哪些邮箱分组」「工作邮箱里有哪些账号」。Agent 通过本地分组目录和账号工具查询空组、未分组及账号归属，结果区分别名和演示记录。这里的分组是司南里的账号收纳文件夹，与邮件服务器上的收件箱、已发送等 IMAP 目录不同；查询分组不需要开启实时邮箱许可。改名或移动账号后，下一次问答会读取更新后的记录。
...
邮箱页默认显示「工作邮箱」「生活邮箱」「未分组」等文件夹，进入文件夹才展开账号。支持自定义名称、颜色、批量移动邮箱，也可按分组名称搜索；删除文件夹只解除归属，不删除账号或邮件。
...
0.6.0 增加安全 HTML 排版、纯文本的 Markdown / GFM 展示及「原文」切换，邮件列表和正文加载时显示骨架屏。正文远程图片默认拦截，点击「加载本封图片」后才请求图片服务器；这可能向发件方泄露 IP 地址和阅读时间。0.9.0 起授权按邮件记忆（最多 500 封），重开同一封邮件不再询问，可在「设置 → 存储」清除；邮件正文与附件在本会话内缓存，重复查看不再重复下载，锁库即清。符合限制的 CID 内嵌图片直接在本机展示，不需要外部请求。
...
0.8.0 的 Chrome / Edge 浏览器扩展可在你主动打开插件时读取当前网页的可见登录表单，核对网站、账号和密码后发送到本机。先在「设置 → 浏览器插件」生成一次性配对码，再到插件中连接；桌面端再次确认后写入主密码库，可直接关联注册邮箱。密码通过认证的本机连接传输，不放进 `nanpad://` 链接；锁库或退出后连接失效，未处理的账号队列也会清除。

插件默认开启「登录时自动采集」（配对后生效，可在弹窗中关闭）：提交含密码的登录表单或点击无表单登录按钮时自动发送到司南待确认队列，页面角落提示结果，桌面端核对后保存，不打断浏览；同一页面重复提交只发送一次。自动采集只读取触发瞬间该表单内的账号与密码，经扩展后台转发，令牌不进入网页。

关闭自动采集后插件没有后台密码监听；任何情况下都不读取浏览器密码库、Cookie 或历史记录。多表单可选择，读不到时可手填；iframe、封闭 Shadow DOM、通行密钥及第三方 OAuth 不属于采集范围。只想记录网站时仍可单独发送站点根地址。下载、配对、权限与排错见 [浏览器插件教程](docs/BROWSER-EXTENSION.md)。
...
- 文档和模型回答中的 Markdown 不执行原始 HTML、不自动加载远程图片；邮件 HTML 在本机净化后展示静态排版，正文外部图片需逐封主动加载。外链只允许 HTTP/HTTPS。
...
安装包输出到 `release/v版本号/`，本版为 `release/v0.10.0/`。`npm run extension:build` 生成配套浏览器扩展目录。打包排除后端测试文件；日常资产、私钥和登录令牌不属于构建输入。发布流程见 [发布说明](docs/RELEASING.md)，本版变化见 [v0.10.0 发布说明](docs/releases/v0.10.0.md)。
