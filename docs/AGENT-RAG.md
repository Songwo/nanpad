...
- 模型得到问题、最多 12 条近期文本消息与命中的片段。资产密码、私钥、密钥值、自由备注、图片和自定义发件人头像映射不参与自动索引；用户导入的文档内容则会参与检索。邮箱的本地分组名称及归属属于可检索元信息，空组与未分组也进入目录。
...
| `list_mail_folders` | 列出全部本地邮箱收纳组，包含空组、未分组及各类账号数量     |
| `list_mailboxes`    | 分页列出邮箱账号与归属，可按本地分组或查询词筛选           |
| `locate_credential` | 仅定位已保存资产的凭据详情入口，不读取或确认凭据存在         |
| `check_mailbox`     | 本次明确授权后，经本机邮箱服务读取实时总数、未读、新增和时间 |

工具不执行 SSH 命令，不修改资产，不发送邮件或推送。`locate_credential` 返回的来源可点击「打开凭据位置」，由用户在资产详情本机解锁查看。模型和凭据定位工具都不能读取加密库；资产存在也不代表已保存密码。

邮箱分组问题先通过 `list_mail_folders` 获取完整目录，再用 `list_mailboxes` 检查成员；相似度检索的前几条命中不能代替完整目录。这两个工具读取本次问答开始时的本机记录，不连接 IMAP、不需要实时邮箱许可。分组改名、删除或账号移动会影响下一次问答。这里的分组不是服务器上的收件箱、已发送等 IMAP 目录，工具不能列出服务端邮件文件夹。

`list_mail_folders` 接受空参数对象，返回分组的 `id`、`name`、`count`、`mailboxCount`、`aliasCount` 和 `demoCount`。`count` 包含邮箱与别名；演示记录也计入总数，`demoCount` 是其中的演示数量，不能再加到总数上。未分组的 `id` 为空字符串。

`list_mailboxes` 的参数均可选：

| 参数       | 含义与限制                                                                 |
| ---------- | -------------------------------------------------------------------------- |
| `folderId` | 本地分组 ID，最多 128 字符；省略表示全部分组，空字符串表示未分组             |
| `query`    | 最多 200 字符，按邮箱名称、地址、域名或分组名称做不区分大小写的包含匹配      |
| `offset`   | 非负整数，默认 0，最大 6000                                                 |
| `limit`    | 每页 1 至 50 个账号，默认 25                                                |

结果包含 `total`、`offset`、`nextOffset` 和 `mailboxes`。`nextOffset` 非空时还存在后续匹配记录；需要完整清单时应继续分页，不能把当前页当作全部账号。账号结果包含资产来源、地址、类型、状态、分组 ID、分组名称及演示标记，不包含密码、正文、草稿或图片。不存在的分组 ID 会返回错误，需重新查询目录。

实时邮箱检查默认关闭。用户发送前勾选「本次允许查询邮箱」，许可随本次请求传入并在下一次发送前重置；主进程独立校验许可，不能通过模型自行调用工具绕过。邮箱服务只连接已保存的真实邮箱，不接受模型指定的新主机、用户名或密码。认证由本机邮箱服务读取已解锁的凭据完成，模型只收到数量和时间，不接触密码、授权码、标题或正文。取消生成、锁库或应用退出会取消在途检查。

首次或重置基线的 `newMessages` 为 `null`，模型必须说明不能据此断言没有新增邮件。未读数量和新增数量含义不同。手动查询与自动推送采用独立检查游标，避免手动查看吃掉下一次推送的新增范围。
...
| `mailbox-baselines.json` | 本机邮箱检查 UID 基线；不包含密码、标题或正文 |

知识库及聊天记录是本机明文数据。导入仅支持 UTF-8 TXT、Markdown，每份最多 500 KB / 100000 字符，共最多 30 份文档，资产、邮箱分组目录和文档合计最多 6000 个索引条目。不要导入密码或密钥。暂不支持 PDF、网页抓取和外部数据库。
...
- `node scripts/website-account-integration.mjs`：验证网站账号加密保存、注册邮箱关联、重载、锁库和导出边界。邮箱服务与推送单测分别覆盖真实 IMAP 协议契约和供应商请求格式，使用独立 fixture，不连接私人账号。
