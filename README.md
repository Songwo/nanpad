...
1. 在 GitHub Releases 下载 `Nanpad-0.6.0-setup.exe`，适用于 Windows x64。
...
安装包输出到 `release/v版本号/`，本版为 `release/v0.6.0/`。`npm run extension:build` 生成配套浏览器扩展目录。打包排除后端测试文件；日常资产、私钥和登录令牌不属于构建输入。发布流程见 [发布说明](docs/RELEASING.md)，本版变化见 [v0.6.0 发布说明](docs/releases/v0.6.0.md)。
