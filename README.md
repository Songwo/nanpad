...
1. 在 GitHub Releases 下载 `Nanpad-0.7.0-setup.exe`，适用于 Windows x64。
...
0.7.0 的 Chrome / Edge 浏览器扩展可在你主动打开插件时读取当前网页的可见登录表单，核对网站、账号和密码后发送到本机。先在「设置 → 浏览器插件」生成一次性配对码，再到插件中连接；桌面端再次确认后写入主密码库，可直接关联注册邮箱。密码通过认证的本机连接传输，不放进 `nanpad://` 链接；锁库或退出后连接失效，未处理的账号队列也会清除。

插件没有后台密码监听，不读取浏览器密码库、Cookie 或历史记录。多表单可选择，读不到时可手填；iframe、封闭 Shadow DOM、通行密钥及第三方 OAuth 不属于采集范围。只想记录网站时仍可单独发送站点根地址。下载、配对、权限与排错见 [浏览器插件教程](docs/BROWSER-EXTENSION.md)。
...
安装包输出到 `release/v版本号/`，本版为 `release/v0.7.0/`。`npm run extension:build` 生成配套浏览器扩展目录。打包排除后端测试文件；日常资产、私钥和登录令牌不属于构建输入。发布流程见 [发布说明](docs/RELEASING.md)，本版变化见 [v0.7.0 发布说明](docs/releases/v0.7.0.md)。
