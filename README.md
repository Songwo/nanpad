...
1. 在 GitHub Releases 下载 `Nanpad-0.2.1-setup.exe`，适用于 Windows x64。
...
**订阅账号**：进入「AI 订阅 → 添加资产 → 快速登录」，选择服务商并点击「网页登录授权」。OpenAI、Grok、Gemini 通常通过本机回调返回；没有自动返回时点击「粘贴回调链接」，提交本次登录产生的完整链接。Claude 可以粘贴完整回调链接或网页给出的 `code#state`。登录成功后自动关联订阅并查询一次用量，后续在详情中刷新。模型设置中也保留授权入口。密码由你在服务商网页输入，Nanpad 不读取浏览器 Cookie。
...
安装包输出到 `release/v版本号/`，本版为 `release/v0.2.1/`。打包排除后端测试文件；日常资产、私钥和登录令牌不属于构建输入。发布流程见 [发布说明](docs/RELEASING.md)，本版变化见 [v0.2.1 发布说明](docs/releases/v0.2.1.md)。
