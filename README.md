...
1. 在 GitHub Releases 下载 `Nanpad-0.8.0-setup.exe`，适用于 Windows x64。
...
| 浏览器插件 | Chrome / Edge 当前网页账号采集、配对本机、核对后加密保存；保留只记录站点入口                            |
...
0.6.0 增加安全 HTML 排版、纯文本的 Markdown / GFM 展示及「原文」切换，邮件列表和正文加载时显示骨架屏。正文远程图片默认拦截，点击「加载本封图片」后才请求图片服务器；这可能向发件方泄露 IP 地址和阅读时间，切换或重新读取邮件后需再次授权。符合限制的 CID 内嵌图片直接在本机展示，不需要外部请求。
...
插件还提供默认关闭的「登录时自动采集」：开启后提交含密码的登录表单会自动发送到司南待确认队列，页面角落提示结果，桌面端核对后保存，不打断浏览。自动采集只读取提交瞬间该表单内的账号与密码，经扩展后台转发，令牌不进入网页。

默认情况下插件没有后台密码监听，不读取浏览器密码库、Cookie 或历史记录。多表单可选择，读不到时可手填；iframe、封闭 Shadow DOM、通行密钥及第三方 OAuth 不属于采集范围。只想记录网站时仍可单独发送站点根地址。下载、配对、权限与排错见 [浏览器插件教程](docs/BROWSER-EXTENSION.md)。
...
安装包输出到 `release/v版本号/`，本版为 `release/v0.8.0/`。`npm run extension:build` 生成配套浏览器扩展目录。打包排除后端测试文件；日常资产、私钥和登录令牌不属于构建输入。发布流程见 [发布说明](docs/RELEASING.md)，本版变化见 [v0.8.0 发布说明](docs/releases/v0.8.0.md)。
